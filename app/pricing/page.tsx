...
export default function PricingPage() {
  ...
  const [simVehicleId, setSimVehicleId] = useState('vt-2');
  ...
  const estimatedPrice = selectedVehicle
    ? Math.max(
        config.defaultMinimumPrice,
        config.baseCharge + simDistance * selectedVehicle.pricePerKm
      )
    : 0;
          ...
          <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Input
                  type="number"
                  value={config.defaultMinimumPrice}
                  onChange={(e) => setConfig({ ...config, defaultMinimumPrice: Number(e.target.value) })}
                  className="h-9 text-sm"
                />
                <p className="text-[11px] text-muted-foreground">Prix plancher appliqué à toutes les courses</p>
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setConfig({ ...config, baseCharge: Number(e.target.value) })}
                  className="h-9 text-sm"
                />
                <p className="text-[11px] text-muted-foreground">Frais fixes à l'acceptation d'une course</p>
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  className="h-9 text-sm"
                />
                <p className="text-[11px] text-muted-foreground">Pourcentage prélevé sur chaque course</p>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-lg bg-muted/50 p-4 space-y-2">
                ...
                <div className="flex justify-between text-xs">
                  ...
                  <span>{formatCurrency(config.baseCharge)}</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-muted-foreground">{simDistance} km × {formatCurrency(selectedVehicle?.pricePerKm ?? 0)}/km</span>
                  <span>{formatCurrency(simDistance * (selectedVehicle?.pricePerKm ?? 0))}</span>
                </div>
                <div className="border-t border-border pt-2 flex justify-between text-xs font-semibold">
                  <span>Sous-total</span>
                  <span>{formatCurrency(config.baseCharge + simDistance * (selectedVehicle?.pricePerKm ?? 0))}</span>
                </div>
                <div className="flex justify-between text-xs text-muted-foreground">
                  <span>Prix minimum appliqué</span>
                  <span>{formatCurrency(config.defaultMinimumPrice)}</span>
                </div>
              </div>

              {/* Result */}
              <div className="rounded-lg border-2 border-primary/20 bg-primary/5 p-4">
                <p className="text-xs text-muted-foreground mb-1">Prix estimé total</p>
                <p className="text-2xl font-bold text-primary">{formatCurrency(estimatedPrice)}</p>
                <div className="mt-3 grid grid-cols-2 gap-2">
                  ...
                  <div className="rounded-md bg-background p-2">
                    <p className="text-[10px] text-muted-foreground">Commission ({config.platformCommission}%)</p>
