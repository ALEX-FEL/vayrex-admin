...
import { payments, getPaymentStats, getPaymentMethodDistribution } from '@/lib/mock-data';
...
export default function PaymentsPage() {
  ...
  const methodDist = getPaymentMethodDistribution();
  ...
  const filtered = useMemo(() => {
    return payments.filter((p) => {
      const matchSearch = search === '' || p.reference.toLowerCase().includes(search.toLowerCase()) || p.clientName.toLowerCase().includes(search.toLowerCase()) || p.driverName.toLowerCase().includes(search.toLowerCase());
    ...
    <DashboardLayout>
      <div className="space-y-5">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <div className="lg:col-span-2">
            <div className="flex gap-3 mb-4">
              <div className="relative flex-1 max-w-xs">
                ...
                <Input placeholder="Rechercher..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-9 h-9 text-sm" />
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="border-b border-border bg-muted/40">
                    <tr>{['Référence', 'Client', 'Chauffeur', 'Montant', 'Mode', 'Date', 'Statut'].map((h) => (<th key={h} className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide whitespace-nowrap">{h}</th>))}</tr>
                  </thead>
                    ...
                    {paginated.map((payment) => (
                      <tr key={payment.id} className="hover:bg-muted/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className={cn('rounded-full border px-2 py-0.5 text-xs font-medium', payment.method === 'CASH' ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-blue-50 text-blue-700 border-blue-200')}>
                            {payment.method === 'CASH' ? 'Cash' : 'Mobile Money'}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-xs text-muted-foreground whitespace-nowrap">{format(payment.createdAt, 'dd/MM/yy HH:mm', { locale: fr })}</td>
          ...
          <div><PaymentMethodChart data={methodDist} /></div>
        </div>
      </div>
    </DashboardLayout>
  );
