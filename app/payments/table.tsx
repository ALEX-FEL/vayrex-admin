...
export function PaymentsTable({ payments }: PaymentsTableProps) {
  ...
  const filtered = useMemo(() => {
    return payments.filter((p) => {
      const matchSearch = search === '' || p.reference.toLowerCase().includes(search.toLowerCase()) || p.client_name.toLowerCase().includes(search.toLowerCase()) || p.driver_name.toLowerCase().includes(search.toLowerCase());
            ...
            <tbody className="divide-y divide-border">
              {paginated.map((payment) => (
                <tr key={payment.id} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs font-medium whitespace-nowrap">{payment.client_name}</td>
                  <td className="px-4 py-3 text-xs text-muted-foreground whitespace-nowrap">{payment.driver_name}</td>
                  ...
                  <td className="px-4 py-3 text-xs text-muted-foreground whitespace-nowrap">{format(new Date(payment.created_at), 'dd/MM/yy HH:mm', { locale: fr })}</td>
