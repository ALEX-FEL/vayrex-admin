...
export default function CoursesPage() {
  ...
  const filtered = useMemo(() => {
    return rides.filter((r) => {
      const matchSearch = search === '' || r.reference.toLowerCase().includes(search.toLowerCase()) || r.clientName.toLowerCase().includes(search.toLowerCase()) || (r.driverName ?? '').toLowerCase().includes(search.toLowerCase()) || r.departure.toLowerCase().includes(search.toLowerCase()) || r.destination.toLowerCase().includes(search.toLowerCase());
  ...
  const exportCSV = () => {
    ...
    const rows = filtered.map((r) => [r.reference, r.clientName, r.driverName ?? '—', r.departure, r.destination, r.distance, r.price, format(r.createdAt, 'dd/MM/yyyy HH:mm'), r.status]);
      ...
      <div className="space-y-5">
        ...
        <div className="flex gap-3">
          <div className="relative flex-1 max-w-sm">
            ...
            <Input placeholder="Rechercher par référence, client..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-9 h-9 text-sm" />
          </div>
          <Select value={statusFilter} onValueChange={(v) => { setStatusFilter(v); setPage(1); }}>
            <SelectTrigger className="w-48 h-9 text-sm"><SelectValue /></SelectTrigger>
            <SelectContent>{statusOptions.map((o) => (<SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>))}</SelectContent>
          </Select>
          <Button onClick={exportCSV} variant="outline" size="sm" className="gap-2"><Download className="h-3.5 w-3.5" />Exporter CSV</Button>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="border-b border-border bg-muted/40">
                <tr>{['Référence', 'Client', 'Chauffeur', 'Départ', 'Destination', 'Distance', 'Prix', 'Date', 'Statut', 'Actions'].map((h) => (<th key={h} className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide whitespace-nowrap">{h}</th>))}</tr>
              </thead>
              <tbody className="divide-y divide-border">
                {paginated.map((ride) => (
                  <tr key={ride.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs text-muted-foreground whitespace-nowrap">{format(ride.createdAt, 'dd/MM/yy HH:mm', { locale: fr })}</td>
                    <td className="px-4 py-3"><StatusBadge status={ride.status} /></td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <Link href={`/courses/${ride.id}`}><Button variant="ghost" size="icon" className="h-7 w-7"><Eye className="h-3.5 w-3.5" /></Button></Link>
                        {ride.status !== 'TERMINÉE' && ride.status !== 'ANNULÉE' && (<Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive"><XCircle className="h-3.5 w-3.5" /></Button>)}
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <div className="flex items-center justify-between border-t border-border px-4 py-3">
            ...
            <div className="flex items-center gap-1">
              ...
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => { const p = Math.max(1, Math.min(totalPages - 4, page - 2)) + i; return <Button key={p} variant={p === page ? 'default' : 'outline'} size="icon" className="h-7 w-7 text-xs" onClick={() => setPage(p)}>{p}</Button>; })}
