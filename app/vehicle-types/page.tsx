...
type VehicleTypeForm = Omit<VehicleType, 'id' | 'createdAt'>;

const defaultForm: VehicleTypeForm = {
  name: '',
  description: '',
  icon: 'car',
  passengerCapacity: 4,
...
export default function VehicleTypesPage() {
  ...
  const handleSave = () => {
    if (editingType) {
      setTypes((prev) => prev.map((t) => t.id === editingType.id ? { ...editingType, ...form } : t));
    } else {
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-0.5 text-sm text-muted-foreground">{types.length} types configurés</p>
          </div>
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input type="number" value={form.passengerCapacity} onChange={(e) => setForm({ ...form, passengerCapacity: Number(e.target.value) })} className="h-9 text-sm" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Prix minimum (XOF)</Label>
                <Input type="number" value={form.minimumPrice} onChange={(e) => setForm({ ...form, minimumPrice: Number(e.target.value) })} className="h-9 text-sm" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Prix par km (XOF)</Label>
                <Input type="number" value={form.pricePerKm} onChange={(e) => setForm({ ...form, pricePerKm: Number(e.target.value) })} className="h-9 text-sm" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Prix par minute (XOF)</Label>
                <Input type="number" value={form.pricePerMinute} onChange={(e) => setForm({ ...form, pricePerMinute: Number(e.target.value) })} className="h-9 text-sm" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Frais de prise en charge (XOF)</Label>
                <Input type="number" value={form.baseCharge} onChange={(e) => setForm({ ...form, baseCharge: Number(e.target.value) })} className="h-9 text-sm" />
              </div>
              <div className="flex items-center gap-3 pt-5">
                <Switch checked={form.isActive} onCheckedChange={(v) => setForm({ ...form, isActive: v })} />
                <Label className="text-xs">{form.isActive ? 'Actif' : 'Inactif'}</Label>
              </div>
            </div>
          </div>
